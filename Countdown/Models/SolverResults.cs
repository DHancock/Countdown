namespace Countdown.Models;

internal sealed class SolverResults
{
    private readonly Lock solvedLock = new();
    private readonly Lock closestLock = new();
    private readonly Lock differenceLock = new();

    // collects a reference of each solver's lists
    private readonly List<List<string>> solvedLists = new(32);
    private readonly List<List<(string, int)>> closestLists = new(32);

    public bool HasSolutions => solvedLists.Count > 0;

    private int difference = SolvingEngine.cNonMatchThreshold;

    // How far from the target that the closest non matching equation is.
    // Shared by all soving engine instances so that they each know when to ignore closest matches.
    public int LowestDifference
    {
        get => difference;

        set
        {
            lock (differenceLock)
            {
                if (difference > value)
                {
                    difference = value;
                }
            }
        }
    }

    // aggregates the solving engine results from each parallel partition
    public void AggregateData(SolvingEngine solvingEngine)
    {
        if (solvingEngine.Solutions.Count > 0)
        {
            lock (solvedLock)
            {
                solvedLists.Add(solvingEngine.Solutions);
            }
        }
        else if ((solvingEngine.Closests.Count > 0) && (LowestDifference > 0))
        {
            lock (closestLock)
            {
                closestLists.Add(solvingEngine.Closests);
            }
        }
    }

    public List<string> GetResults()
    {
        int size = 0;
        List<string> results;

        if (solvedLists.Count > 0)
        {
            foreach (List<string> solvedResults in solvedLists)
            {
                size += solvedResults.Count;
            }

            results = new(size);

            foreach (List<string> solvedResults in solvedLists)
            {
                results.AddRange(solvedResults);
            }

            return results;
        }

        size += 2; // allow space for any preamble

        foreach (List<(string, int difference)> closestResults in closestLists)
        {
            foreach ((string equation, int difference) in closestResults)
            {
                if (difference == LowestDifference)
                {
                    ++size;
                }
            }
        }

        results = new(size);

        foreach (List<(string equation, int difference)> closestResults in closestLists)
        {
            foreach ((string equation, int difference) in closestResults)
            {
                if (difference == LowestDifference)
                {
                    results.Add(equation);
                }
            }
        }

        return results;
    }
}
